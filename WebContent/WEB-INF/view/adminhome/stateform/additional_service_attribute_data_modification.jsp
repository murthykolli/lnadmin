<%--
	Document : additional_service_attribute_data_modification
 ...
 /* State and Form selection code */ 
    	function stateNameValidation(){    		
 		...
 		function formNameValidation(){
    		var stateName = document.getElementById("stateNameId").value; var formName = document.getElementById("formNameId").value;
            var stateName1 = '<c:out value="${stateName}"/>'; var formName1 = '<c:out value="${formName}"/>';
            var formHeading = '----Select Form Name---';
 		...
 		function removeDuplicates(obj){    	 	
            var uniques=[]; var stringify={};
            for(var i=0;i<obj.length;i++){        	
            var keys = Object.keys(obj[i]); var str='';
            for(var j=0;j<keys.length;j++){       
            str+= JSON.stringify(keys[j]); str+= JSON.stringify(obj[i][keys[j]]); }
            if(!stringify.hasOwnProperty(str)){
            uniques.push(obj[i]); stringify[str]=true; } }
    	...
    	function newAjaxStateCall(selectedstate){   
        	$.ajax({
    			...
    			cache: false,
    			success : function(result) {
    				$("#FormsDropDownId").show();
    				$('#formNameId').empty();
    				var jsonStrObj = removeDuplicates(result); var data = JSON.parse(jsonStrObj); var formName = '<c:out value="${formName}"/>'; var stateName = '<c:out value="${stateName}"/>';
    				var selectHeading;
    	...
    	function newAjaxFormCall(selectedstate){    		
        	$.ajax({
    			...
    			cache: false,
    			success : function(result) {
    				window.location.href = "jSonAdditionalFormsAttrDataDisplay";    				
    			} }); }    	
    </script>
    </head>    
        <body>
		...
		<div>
		<div><font id="myAccountDocText">Additional Forms - <font id="myAccountDocCategory">Attributes Modification</font></font></div>
        ...
        <%    
        if (null != session.getAttribute("Additional_Forms_Attr_Info_ID_List")) {
        ArrayList attrInfoIdList = (ArrayList) session.getAttribute("Additional_Forms_Attr_Info_ID_List");
        if (null != attrInfoIdList && attrInfoIdList.size() > 0) {
        	
        if (null != session.getAttribute("Additional_Forms_Attr_Field_Names_List")) {
        ArrayList attrFieldNamesList = (ArrayList) session.getAttribute("Additional_Forms_Attr_Field_Names_List");
