<%--
	Document : federal_tax_attribute_data_modification
...
<html>    
    ...
    function attributesSaveIds(id){
        ...
        function newAjaxCall(selectedData){        	
        	$.ajax({
    			type : 'POST',
    			url : "/lnadmin/federalTaxAttrFieldNameUpdate?attributeFieldName="+selectedData,
    			dataType : 'json',
    			async : true,
    			cache: false,
		...
		<div>
		<div><font id="myAccountDocText">Federal Tax ID - <font id="myAccountDocCategory">Attributes Modification</font></font></div>
		<div class="row">
		...
		<h2><img class="aln-st-img-adm" src="resources/images/FormNameImage.png" alt="">&nbsp; Federal Tax ID (EIN) &nbsp;</h2>
		
		<div class="clearfix"></div>
		</div>
		
        <% 
        if (null != session.getAttribute("Fed_Attr_Info_ID_List")) {
        ArrayList attrInfoIdList = (ArrayList) session.getAttribute("Fed_Attr_Info_ID_List");
        if (null != attrInfoIdList && attrInfoIdList.size() > 0) {
        	
        if (null != session.getAttribute("Fed_Attr_Field_Names_List")) {
        ArrayList attrFieldNamesList = (ArrayList) session.getAttribute("Fed_Attr_Field_Names_List");
