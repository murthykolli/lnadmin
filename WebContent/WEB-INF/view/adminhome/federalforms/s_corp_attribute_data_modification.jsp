<%--
	Document : s_corp_attribute_data_modification
...
<html>    
    ...
    function attributesSaveIds(id){
        ...
        function newAjaxCall(selectedData){        	
        	$.ajax({
    			type : 'POST',
    			url : "sCorpAttrFieldNameUpdate?attributeFieldName="+selectedData,
    			dataType : 'json',
    			async : true,
    			cache: false,
		...
		<div>
		<div><font id="myAccountDocText">S Corporation - <font id="myAccountDocCategory">Attributes Modification</font></font></div>
		<div class="row">
		...
		<h2><img class="aln-st-img-adm" src="resources/images/FormNameImage.png" alt="">&nbsp; S Corporation &nbsp;</h2>
		
		<div class="clearfix"></div>
		</div>
		
        <% 
        if (null != session.getAttribute("SCorp_Attr_Info_ID_List")) {
        ArrayList attrInfoIdList = (ArrayList) session.getAttribute("SCorp_Attr_Info_ID_List");
        if (null != attrInfoIdList && attrInfoIdList.size() > 0) {
        	
        if (null != session.getAttribute("SCorp_Attr_Field_Names_List")) {
        ArrayList attrFieldNamesList = (ArrayList) session.getAttribute("SCorp_Attr_Field_Names_List");
