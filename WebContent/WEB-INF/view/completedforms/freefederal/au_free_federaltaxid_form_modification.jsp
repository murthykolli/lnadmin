<%--
    Document   : free_FederalTaxForm_Modification
...
<html>    
    ...
    var hidingClassNames = '<c:out value="${FreeFedRadioButtonStatus}"/>';    
    ...
    var formPageValues = '<c:out value="${freeFederalFormPageValues}"/>';    
    var boxes = $(".box");
    ...
    var attrTFList = '<c:out value="${freeFedAttributeTextFieldList}"/>';	
	var attrTFDateList = '<c:out value="${freeFedAttributeTextFieldDateList}"/>';
	var attrSBoxList = '<c:out value="${freeFedAttributeSelectBoxList}"/>';
	var attrRButtonList = '<c:out value="${freeFedRadioButtonList}"/>';
	var attrCheBoxList = '<c:out value="${freeFedCheckBoxList}"/>';
	var firstChoice = '<c:out value="${firstChoice}"/>';
    ...
    if(attrTFList1[0] === ""){    	
    	$("[name=attributeTextField1]").val(convert(firstChoice));	
    } else{
    	$("[name=attributeTextField1]").val(convert(attrTFList1[0]));
    }
    for(var i=1;i<attrTFList1.length;i++){
    ...
    if($("[name=attributeSelectBox4]").val()==="Other"){ $(".sother").show(); }
		...
		<form action="compFreeFederalDataModification" method="post" name="federaltax" id="pro-form">            
            ...
            <tr class="<%=attStatusList.get(i)%>" style="text-align: justify;"><td id="attribute<%=i%>"><%=attNamesCorpList.get(i)%></td></tr> <tr class="<%=attStatusList.get(i)%>" style=" height: 7px;"><td></td></tr>
            ...
            <% } %> </table> </div>            
            <% } } } } } } } } } %>
            <input type="hidden" name="freeFederalFormPageValues"/>
            <input type="hidden" name="freeFederalDocFinishOrderRef" />            
        ...
        <script type="text/javascript" src="resources/scripts/federalforms/formfederaltax.js"></script>
